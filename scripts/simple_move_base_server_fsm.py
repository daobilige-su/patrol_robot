...
class simple_move_base_action(object):
    ...
    def execute_cb(self, goal):
        ...
        while success!=True:
            ...
            if mode == 1:  # first turning with theta1
                if abs(pose_diff_M_2d_line_yaw)<pose_diff_2d_line_yaw_thr:
                    mode = 2 # travels to the goal position
                    cmd_vel_x_diff = pose_diff_M_2d_line_dist
                    cmd_vel_theta_diff = pose_diff_M_2d_line_yaw
                else:
                    cmd_vel_x_diff = 0.0
                    cmd_vel_theta_diff = pose_diff_M_2d_line_yaw
            elif mode == 2:  # travels to the goal position
                if pose_diff_M_2d_line_dist < pose_diff_dist_thr:
                    mode = 3  # second turning with theta2
                    cmd_vel_x_diff = 0.0
                    cmd_vel_theta_diff = pose_diff_trans_ypr[3, 0]
                else:
                    cmd_vel_x_diff = pose_diff_M_2d_line_dist
                    cmd_vel_theta_diff = pose_diff_M_2d_line_yaw
            elif mode == 3:  # second turning with theta2
                if abs(pose_diff_trans_ypr[3,0])<pose_diff_yaw_thr:
                    mode = 4 # rearched the goal position
                    cmd_vel_x_diff = 0.0
                    cmd_vel_theta_diff = 0.0
                    success = True
                else:
            ...
            #         cmd_vel_theta_diff = pose_diff_M_2d_line_yaw
            #     else:
            #         mode = 1 # first turning with theta1
            #         cmd_vel_x_diff = 0.0
            #         cmd_vel_theta_diff = pose_diff_M_2d_line_yaw
            ...
            if cmd_vel_x>cmd_vel_x_max:
                cmd_vel_x = cmd_vel_x_max
            elif cmd_vel_x<(-1.0*cmd_vel_x_max):
                cmd_vel_x = -1.0*cmd_vel_x_max
